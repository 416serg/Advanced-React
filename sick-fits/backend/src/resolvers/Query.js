...
const Query = {
  ...
  async order(parent, args, ctx, info) {
    ...
    if (!ctx.request.userId) throw new Error('You must be logged in to view order');
  ...
  async orders(parent, args, ctx, info) {
    ...
    if (!userId) throw new Error('You must be logged in to view order');
    // 2. Query the current order
    const orders = await ctx.db.query.orders({ where: { user: { id: userId } } }, info);
    return orders;
  },
};
